...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        def helper(place, distance=0):
            if place == hive:
                return
            elif place.bees and ((self.max_range is None and self.min_range is None) or (self.max_range is not None and self.max_range >= distance) or (self.min_range is not None and self.min_range <= distance)):
                return place.bees
            return helper(place.entrance, distance+1)
        return random_or_none(helper(self.place))
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def buff_behind(self, place):
        if place is None:
            return
        cur_ant = place.ant
        if cur_ant is not None:
            self.buff_ant(cur_ant)
            if cur_ant.is_container and cur_ant.contained_ant is not None:
                self.buff_ant(cur_ant.contained_ant)
        self.buff_behind(place.exit)
    ...
    def action(self, colony):
        ...
        if self.is_true_queen == False:
            ...
            super().action(colony)
            self.buff_behind(self.place.exit)
        # END Problem 13
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if target and not target.scared_once:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, hive):
        # BEGIN Problem OPTIONAL
        cur, insects, distance = self.place, {}, 0
        while cur != None:
            if cur.ant is not None and cur.ant is not self:
                insects[cur.ant] = distance
                if cur.ant.is_container and cur.ant.contained_ant is not None:
                    insects[cur.ant.contained_ant] = distance
            if cur.bees:
                for bee in cur.bees:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        real_damage = self.damage-distance*0.2-self.insects_shot*0.05
        if real_damage:
            return real_damage
        return 0
        # END Problem OPTIONAL
