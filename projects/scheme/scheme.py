...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        oprands = rest.map(lambda arg: scheme_eval(arg, env))
        return scheme_apply(op, oprands, env)
        # END PROBLEM 5
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('formal parameters dont match argument values')
        child = Frame(self)
        while formals:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        procedure_name, formals, body = target.first, target.second, expressions.second
...
def do_define_macro(expressions, env):
    ...
    macro_name, formal = target.first, target.second
    body = expressions.second
    env.define(macro_name, MacroProcedure(formal, body, env))
    return macro_name
    # END Problem 21
