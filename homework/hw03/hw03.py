...
def totals_tree(m):
    ...
    leftx, rightx = [], []
    leftx = totals_tree(end(left(m)))
    rightx += totals_tree(end(right(m)))
    return tree(total_weight(m), [leftx, rightx])
...
def pow_church(m, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    return lambda f: lambda x: m(n(f))(m(f)(x))
    # more elegant way: return n(m)
