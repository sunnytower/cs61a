...
def make_withdraw(balance, password):
    ...
    def helper(amount,try_password):
        nonlocal wrong_passwd,balance
        if len(wrong_passwd)==3:
            return "Your account is locked. Attempts: "+str(wrong_passwd)
        if try_password!=password:
            wrong_passwd.append(try_password)
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        temp = Mint.current_year-self.year-50
        if temp>0:
